...
import SearchProduct from "./SearhProduct";
import FilterShop from "./FilterShop";
import { Navigate } from "react-router-dom";
import { useSelector,useDispatch } from "react-redux";
import {setProductsState, setUniqueBrandState, setCopyUniqueBrandState} from '../store/dataSlice';


function Shop({storeName}) {
    ...
    useEffect(() => {
        axios('https://restcountries.com/v2/all').then(data => { // данные с сайта не смог выгрузить, везде ошибка (Причина: отсутствует заголовок CORS «Access-Control-Allow-Origin»)
    ...
    useEffect( () => {
        dispatch(setUniqueBrandState(products.reduce((acc, product) => {
    ...
    useEffect( () => {
        dispatch(setCopyUniqueBrandState(copyUniqueBrand.length ? copyUniqueBrand : products.reduce((acc, product) => { //сделал копию т.к. фильтр бы сбивался при добавлении/удалении в/из корзины
            if(!acc.includes(product.brand)) {
            acc.push(product.brand);
            }
             return acc; }, [])
    ...
    function selectProduct(id, value) {
        dispatch(setProductsState(products.map(product => {
            if(product.id === id) {
                return {...product, selected: value, amount: 1};
            }
            return {...product};
        })))
    }

    function selectAmount (id, increase) {
        dispatch(setProductsState(products.map(product => {
            if(product.id === id) {
                return (increase ? {...product, amount: product.amount +1} : {...product, amount: (product.amount > 0 ? product.amount - 1 : product.amount)});
            }
            return {...product};
        })))
    }
    ...
    function processSearch(e) {
        ...
        dispatch(setProductsState(products.map(product => (product.model.toLowerCase().includes(search) || product.brand.toLowerCase().includes(search)) ? 
    ...
    <h1>Products</h1>
    <SearchProduct processSearch={processSearch}/>

    <Stack direction="horizontal" gap={2}>
  <div style={{width: '14rem'}} className="bg-light border flex-column"><FilterShop /></div>

  <div className="vr" />
  <div className="bg-light border flex-column"><Cart selectProduct={selectProduct} selectAmount={selectAmount}/>
  <Container>
        <Row>
            { 
            (products.filter(product => (product.price < priceMax ) && 
            (product.price > priceMin) &&
            (copyUniqueBrand.filter(brand => brand == product.brand).length))
            ).map(product => <Product
